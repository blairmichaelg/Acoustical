...
# 🎸 Acoustical - Acoustic Cover Assistant

[![CI](https://github.com/blairmichaelg/Acoustical/actions/workflows/python-app.yml/badge.svg)](https://github.com/blairmichaelg/Acoustical/actions/workflows/python-app.yml)
...
**A powerful, cloud-enhanced open-source tool for singer/songwriters**  
Effortlessly analyze, adapt, and creatively transform songs with AI-powered chord extraction, lyrics retrieval, and musical flourishes.
...
Empower beginner to intermediate singer/songwriters to **think of any song and instantly get the chords, lyrics, and creative progression ideas**. Acoustical helps you put your own unique spin on covers or build new compositions, all powered by a robust, scalable cloud backend.

> **Note:** This project is enhanced with Google Cloud Platform for improved scalability, AI capabilities, and as a learning experience.

## ✨ Features

### 🎵 Core Capabilities
- **🎼 Intelligent Chord Extraction** - Extract chords from audio files or YouTube URLs using multiple backends (Essentia, Chordino, AutoChord)
- **📝 Lyrics Retrieval** - Fetch song lyrics automatically from URLs or metadata
- **🔑 Key Detection & Analysis** - Automatically detect the musical key of any song
- **🎹 Chord Transposition** - Transpose songs to any key with music theory accuracy
- **🎸 Capo Recommendations** - Get smart capo position suggestions for easier playing
- **🎨 Creative Flourishes** - Generate musical embellishments using AI (Magenta, LLM) or rule-based algorithms
- **🖐️ Fingering Suggestions** - Receive guitar fingering advice for chord progressions

### ⚡ Technical Features
- **🔌 Plugin Architecture** - Extensible backend system for chord extraction and flourish generation
- **🌐 Web & CLI Interfaces** - Use the Flask web app or command-line interface
- **☁️ Cloud-Enhanced** - Scalable deployment on Google Cloud Platform (Cloud Run, Vertex AI)
...
![Click](https://img.shields.io/badge/Click-CLI-blue?style=flat)

### Music & Audio Processing
- **music21** - Music theory analysis and transposition
- **aubio** - Audio analysis and chord detection (Windows default)
- **Essentia** - Advanced audio analysis (optional, requires manual install)
- **Magenta** - AI-powered musical flourish generation

### Cloud Infrastructure (Optional)
- **Google Cloud Run** - Serverless web app hosting
- **Vertex AI** - ML model deployment (Essentia, Magenta)
- **Cloud Storage** - Audio file storage
- **Cloud Functions** - Microservices orchestration
...
- (Optional) yt-dlp for YouTube audio download
...
1. Navigate to `http://localhost:5000` after starting the Flask app
2. Upload an audio file or paste a YouTube URL
3. Extract chords, retrieve lyrics, transpose, or generate flourishes
4. Download results as JSON

### Command Line Interface

The CLI provides powerful commands for batch processing and automation:

```bash
# Extract chords from a local file
acoustical extract-chords path/to/song.mp3

# Extract chords from a YouTube URL
acoustical extract-chords "https://www.youtube.com/watch?v=..."

# Detect the key of a song
acoustical key --chords path/to/chords.json

# Transpose chords
acoustical transpose --input chords.json --semitones 2

# Get capo recommendations
acoustical capo --input chords.json

# Generate flourishes
acoustical flourish --input chords.json --backend magenta

# Download audio from YouTube
acoustical download-audio "https://www.youtube.com/watch?v=..." --out_dir ./audio

# Check available backends
acoustical check-backends
```

### Python API

```python
from chord_extraction import get_chords, check_backend_availability
...
# Extract chords from file
chords = get_chords("song.mp3")

# Extract chords from URL
chords_from_url = get_chords("https://www.youtube.com/watch?v=...")

# Detect key
key_info = detect_key_from_chords(chords)

# Transpose
transposed = transpose_chords(chords, semitones=2)
...
Acoustical leverages Google Cloud Platform for scalability and AI capabilities:

| Service | Purpose |
|---------|---------|
| **Cloud Run** | Serverless Flask web app hosting |
| **Cloud Storage** | Audio uploads and temporary processing files |
| **Vertex AI** | Custom ML models (Essentia chord extraction, Magenta flourishes) |
| **Cloud Functions** | LLM chord substitutions, lyrics retrieval |
...
2. **Configure GCP Project**
   - Enable required APIs: Cloud Run, Vertex AI, Cloud Functions, Cloud Build, Artifact Registry, Cloud Storage
...
4. **Access Application**
   - Cloud Run will provide a public URL for the web app

See [ARCHITECTURE.md](ARCHITECTURE.md) for detailed cloud architecture information.
